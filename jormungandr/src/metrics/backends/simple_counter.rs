...
impl MetricsBackend for SimpleCounter {
    ...
    fn add_tx_pending_cnt(&self, count: usize) {
        self.tx_pending_cnt.fetch_add(count, Ordering::SeqCst);
    }

    fn sub_tx_pending_cnt(&self, count: usize) {
        self.tx_pending_cnt.fetch_sub(count, Ordering::SeqCst);
    }
